...
class GzipDecoder:
    ...
    def decode(self, content: bytes) -> List[str]:
        ...
        try:
            ...
            logger.info(
                f"Successfully decompressed gzip content: {len(decompressed)} bytes"
            )
...
class PlainTextDecoder:
    ...
    def decode(self, content: bytes) -> List[str]:
        ...
        try:
            ...
            valid_schemes = ["ss://", "trojan://", "vmess://", "vless://", "anytls://"]
            if not any(
                any(url.strip().startswith(scheme) for scheme in valid_schemes)
                for url in urls
            ):
                raise DecodingError(
                    "Content doesn't appear to contain valid proxy URLs"
                )

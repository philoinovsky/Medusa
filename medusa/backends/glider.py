    ...
    def convert(self, parsed_url: ParseResult) -> str:
        ...
        if parsed_url.username is None:
            try:
                ...
                parsed_url = urlparse(
                    urlunparse(parsed_url).replace(parsed_url.netloc, netloc)
                )
        ...
        if parsed_url.password is None and parsed_url.username:
            try:
                ...
                parsed_url = urlparse(
                    urlunparse(parsed_url).replace(parsed_url.username, username)
                )
            except Exception as e:
    ...
    def convert(self, parsed_url: ParseResult) -> str:
        ...
        result = "".join(
            [
                ...
                "&skipVerify=true" if sni else "skipVerify=true",
                "#",
                unquote(parsed_url.fragment) if parsed_url.fragment else "",
            ]
        )
...
class AnyTLSHandler(ProxyHandler):
    """Handler for AnyTLS URLs."""
    ...
    def convert(self, parsed_url: ParseResult) -> str:
        """Convert anytls:// URL to Glider format."""
        self.validate(parsed_url)

        if not parsed_url.username:
            raise BackendError("Missing password in anytls:// URL")
        ...
        skip_verify = "true" if insecure == "1" else "false"
        ...
        result = "".join(
            [
                ...
                "?",
                f"serverName={sni}" if sni else "",
                f"&skipVerify={skip_verify}" if sni else f"skipVerify={skip_verify}",
                "#",
                unquote(parsed_url.fragment) if parsed_url.fragment else "",
            ]
        )
